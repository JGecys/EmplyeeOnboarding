...
public class StopWords
{
  ...
  public static final StringReplacement PUNCTUATION = StringReplacement.compile("[^\\p{Ll}\\p{Lu}\\p{Lt}\\p{Lo}\\p{Nd}\\p{Pc}\\s]", "");
  ...
  public static WordStats getStopWordCount(String content) {
	  if (StringHelper.isNullOrEmpty(content)) return WordStats.EMPTY;

	  WordStats ws = new WordStats();

	  String strippedInput = removePunctuation(content);
	  String[] words = StringHelper.SPACE_SPLITTER.split(strippedInput);
	  ...
	  {
		  String word = words[i];
		  if (StringHelper.isNullOrEmpty(word)) continue;
  ...
  public static String removeStopWords(String str) {
	  return str.replaceAll(STOP_WORDS.toString(), "");
  }
